...
async function generateAsciiArt(text) {
  try {
    const response = await fetch(
      `https://asciified.thelicato.io/api/v2/ascii?text=${encodeURIComponent(
        text
      )}&font=pagga`
    );
...
document.addEventListener("mousemove", (e) => {
  if (volumeHandle.classList.contains("dragging")) updateVolume(e.clientX);
});
...
document.addEventListener("touchmove", (e) => {
  if (volumeHandle.classList.contains("dragging")) {
    updateVolume(e.touches[0].clientX);
    e.preventDefault();
  }
});
...
audio.volume = 0.5;
updateVolume(
  volumeBar.getBoundingClientRect().left + volumeBar.offsetWidth * 0.5
);
...
document.addEventListener("DOMContentLoaded", () => {
  ...
  document.querySelectorAll(".typewrite").forEach((el) => {
    el.setAttribute(
      "data-type",
      JSON.stringify(CONFIG.profile.subtitleRotations)
    );
  ...
  if (socialGrid) {
    const items = [
      {
        href: CONFIG.social.discordUser,
        cls: "discord",
      ...
      {
        href: CONFIG.social.valorant,
        cls: "valorant",
        icon: '<img src="assets/icons/valorant.png" alt="Valorant">',
        label: "Valorant",
      },
      {
        href: CONFIG.social.roblox,
        cls: "roblox",
        icon: '<img src="assets/icons/roblox.png" alt="Roblox">',
        label: "Roblox",
      },
    ];
  ...
  // Payment button
  document
    .querySelectorAll(".show-payment-info")
    .forEach((el) => el.addEventListener("click", showPaymentInfo));
